...
const initSchema = () => {
    ...
    const finalColumns = db.prepare("PRAGMA table_info(items)").all().map((c: any) => c.name);
    ...
    if (!hasManga) {
        ...
        const transaction = db.transaction(() => {
            ...
            const oldColumns = db.prepare("PRAGMA table_info(items_old)").all().map((c: any) => c.name);
            const validColumns = [
...
const existingColumns = db.prepare('PRAGMA table_info(items)').all().map((c: any) => c.name);
...
ipcMain.handle('delete-item', (event: any, id: any) => {
    try {
...
ipcMain.handle('import-from-text', async (event: any) => {
    try {
...
ipcMain.handle('bulk-add-items', (event: any, { titles, category, status }: any) => {
    const insert = db.prepare(`
        INSERT INTO items (title, category, status) VALUES (?, ?, ?)
    `);

    const importTransaction = db.transaction((titles: any) => {
...
ipcMain.handle('backup-restore', async () => {
    try {
        ...
        // Verification
        const hasDb = zipEntries.some((entry: any) => entry.entryName === 'media_tracker.db');
        if (!hasDb) {
        ...
        const hasCovers = zipEntries.some((entry: any) => entry.entryName.startsWith('covers/'));
        if (hasCovers) {
...
const getSettingWithEnv = (key: string, envName?: string): string | null => {
    try {
...
ipcMain.handle('tmdb-search', async (event: any, query: any, category: any) => {
    try {
        ...
        if (!apiKey) {
            const err: any = new Error('TMDB API Key is missing. Please configure it in Settings.');
            err.code = 'NO_API_KEY';
            throw err;
        }
...
ipcMain.handle('books-search', async (event: any, query: any) => {
    try {
...
ipcMain.handle('books-get-details', async (event: any, id: any) => {
    try {
        const apiKey = getSettingWithEnv('google_books_api_key', 'GOOGLE_BOOKS_API_KEY');
        const details: any = await books.getDetails(id, apiKey);
...
ipcMain.handle('anilist-search', async (event: any, query: any, category: any) => {
    try {
...
ipcMain.handle('anilist-get-details', async (event: any, id: any, category: any) => {
    try {
        const details: any = await anilist.getDetails(id, category || 'Anime');
