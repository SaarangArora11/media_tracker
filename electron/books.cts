...
const makeRequest = async (url: string, attempts = 3): Promise<any> => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const err: any = new Error(`Google Books API Error [Code ${response.status}]: ${statusMsg}`);
        ...
        if (data !== null) {
            ...
            const err: any = new Error(`Google Books Error [Code ${response.status}]: Invalid JSON response`);
            err.code = response.status;
            throw err;
        }
    } catch (err: any) {
...
const getOpenLibraryDetails = async (workId: string) => {
...
module.exports = {
    search: async (query: string, apiKey?: string) => {
        ...
        try {
            const data: any = await makeRequest(url);
    ...
    getDetails: async (id: string, apiKey?: string) => {
