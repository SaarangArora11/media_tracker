...
const fs = require('fs');
const makeGraphQLRequest = async (query, variables, attempts = 3) => {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            const err = new Error(`AniList API Error [Code ${response.status}]: ${errorMsg}`);
        ...
        if (data?.errors && data.errors.length > 0) {
            ...
            const err = new Error(`AniList GraphQL Error: ${errorMsg}`);
        ...
        if (data?.data) {
            ...
            const err = new Error(`AniList Error: Invalid response structure`);
...
const parseMediaItem = (item, defaultCategory) => {
...
`;
module.exports = {
    search: async (query, category = 'Anime') => {
        ...
        return items.map((item) => parseMediaItem(item, category));
    },
    getDetails: async (id, category = 'Anime') => {
        const data = await makeGraphQLRequest(DETAILS_QUERY, {
            id: Number(id)
        });
        const item = data?.Media;
        if (!item)
            throw new Error(`Media not found for AniList ID ${id}`);
